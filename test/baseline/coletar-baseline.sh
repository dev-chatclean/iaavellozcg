...
export FERIADOS=""
# Transferencia de departamento (introduzida na develop). Fixadas aqui para a
# coleta nao depender do ambiente da maquina; os IDs caem no padrao do data.js.
export TRANSFERIR_DEPARTAMENTO="true"
export TRANSFERIR_FECHANDO="false"
export DEPT_ID_MATRIZ=""
export DEPT_ID_MALVINAS=""
export DEPT_ID_MONTEIRO=""
export DEPT_ID_AGENTE_IA=""
export DEPT_ID_POSVENDA=""
...
req "GET /diag/transferir sem chave"  "http://localhost:$PORT/diag/transferir"
req "GET /diag/transferir com chave"  "http://localhost:$PORT/diag/transferir?key=chave-baseline&numero=5583999999999&loja=malvinas"
...
#   3. o /health devolve uptime e timestamp, que mudam a cada execucao.
node -e '
const fs = require("fs");
...
texto = texto.replace(/"uptime":[\d.]+,"timestamp":"[^"]*"/g, "\"uptime\":\"(normalizado)\",\"timestamp\":\"(normalizado)\"");
